...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    if (options[:month_header])
      ...
      if options[:all_event]
        if options[:year]==Date.today.strftime("%Y").to_i
        cal << %(<th colspan="#{colspan}" style="width:120px;" class="#{options[:month_name_class]}"><a style="color:black;" href="javascript:all_events(#{options[:month]});">#{Date::MONTHNAMES[options[:month]]}</a></th>)
        else
          cal << %(<th colspan="#{colspan}" style="width:120px;" class="#{options[:month_name_class]}"><a style="color:black;" href="javascript:all_events(#{options[:month]});">#{Date::MONTHNAMES[options[:month]]} #{options[:year].to_s[2..3]}</a></th>)
        end
      else
        if options[:year]==Date.today.strftime("%Y").to_i
          cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{Date::MONTHNAMES[options[:month]]}</th>)
        else
          cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{Date::MONTHNAMES[options[:month]]} #{options[:year].to_s[2..3]}</th>)
        end
      end
