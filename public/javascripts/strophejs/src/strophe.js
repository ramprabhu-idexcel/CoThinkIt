    ...
    */
    ...
    _processRequest: function (i)
    {
        ...
        if (req.xhr.readyState === 0) {
            ...
            req.date = new Date();
            try {
                // 2009-10-26 AN the disconnect call from safari needs to be synchronous
                var async = true;
                if( leaving_safari )
                  async = false;
                req.xhr.open("POST", this.service, async);
                // 2009-10-26 AN just in case; set back leaving_safari to false
                if( leaving_safari )
                  leaving_safari = false;
            } catch (e) {
